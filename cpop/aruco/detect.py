...
class ArucoDetector:
    ...
    def draw_ar_cubes(self, img_rgb, aruco_poses: ArucoPoseDetections) -> np.array:
        ...
        for i in range(len(rvecs)):
            ...
            sides = cv2.drawContours(sides, [np.array([imgpts[0], imgpts[1], imgpts[5], imgpts[4]])], -1, color, -2)
            # Draw the left side vertical to the marker
            sides = cv2.drawContours(sides, [np.array([imgpts[2], imgpts[3], imgpts[7], imgpts[6]])], -1, color, -2)
            # Draw the front side vertical to the marker
            sides = cv2.drawContours(sides, [np.array([imgpts[1], imgpts[2], imgpts[6], imgpts[5]])], -1, color, -2)
            # Draw the back side vertical to the marker
            sides = cv2.drawContours(sides, [np.array([imgpts[0], imgpts[3], imgpts[7], imgpts[4]])], -1, color, -2)
            # Until here the walls of the cube are drawn in and can be merged
            img_rgb = cv2.addWeighted(sides, 0.1, img_rgb, 0.9, 0)
